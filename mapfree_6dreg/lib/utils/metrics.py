...
def pose_error_torch(R, t, Tgt, reduce=None):
    ...
    # R: [B, 3, 3] t: [B, 3]  Tgt: [B, 4, 4]

    Rgt = Tgt[:, :3, :3]                  # [B, 3, 3]
    tgt = Tgt[:, :3, 3:].transpose(1, 2)  # [B, 1, 3]

    scale_t = torch.linalg.norm(t, dim=-1) # B
    scale_tgt = torch.linalg.norm(tgt, dim=-1) # B, 1

    cosine = (torch.bmm(t.unsqueeze(1), tgt.transpose(1, 2))).squeeze(-1) / (scale_t * scale_tgt.squeeze(-1) + 1e-9).unsqueeze(1) # B,B
    cosine = torch.clip(cosine, -1.0, 1.0)    # handle numerical errors
    t_ang_err = torch.rad2deg(torch.acos(cosine)) # B, B
    ...
    residual = R.transpose(1, 2) @ Rgt # B, 3, 3
    trace = torch.diagonal(residual, dim1=-2, dim2=-1).sum(-1) # B
    cosine = (trace - 1) / 2 # B
    cosine = torch.clip(cosine, -1., 1.)  # handle numerical errors
    R_err = torch.rad2deg(torch.acos(cosine)) # B
