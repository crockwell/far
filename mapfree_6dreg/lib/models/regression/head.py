...
class DeepResBlock(torch.nn.Module):
    def __init__(self, cfg, in_channels, full_forward_pass=True):
        super().__init__()
        ...
        self.full_forward_pass = full_forward_pass
        if full_forward_pass:
            self.resblock4 = PreActBlock(256, 512, stride=2, bn=bn)

    def forward(self, feature_volume):
        B = feature_volume.shape[0] # here shape B, 67, 92, 68

        x = self.resblock1(feature_volume) # now shape B, 64, 46, 34
        x = self.resblock2(x) # now shape B, 128, 23, 17
        x3 = self.resblock3(x) # now shape B, 128, 12, 9
        if self.full_forward_pass:
            x = self.resblock4(x3)
...
class ProcrustesDeepResBlock(DeepResBlock):
    ...
    def forward(self, feature_volume, data):
        B = feature_volume.shape[0] # B, 67, 60, 80
        x = super().forward(feature_volume) # B, 512
        xyz = self.mlp(x).view(B, -1, 3) # B, 6, 3
...
class QuatDeepResBlock(DeepResBlock):
    ...
    def forward(self, feature_volume, data):
        ...
        data['q'] = quat
        import kornia
        if kornia.__version__() == 'kornia-0.7.1.dev0':
...
class DirectDeepResBlockMLP(DeepResBlock):
    ...
    def __init__(self, cfg, in_channels, full_forward_pass=True):
        super().__init__(cfg, in_channels, full_forward_pass)
    ...
    def forward(self, feature_volume, data):
        B = feature_volume.shape[0]
        x, x3 = super().forward(feature_volume)
        if self.full_forward_pass:
            out = self.mlp(x).view(B, 9)
