...
class EssentialMatrixSolver:
    ...
    def __init__(self, cfg, use_prior_ransac=False):
        ...
        self.use_prior_ransac = use_prior_ransac

    def estimate_pose(self, kpts0, kpts1, data, priorRT=None):
        R = np.eye(3)
        t = np.zeros((3))
        if len(kpts0) < 5:
            return (R, t, 0), 0, 0
        ...
        if self.use_prior_ransac and priorRT is not None:
            ...
            prior_params = {
                ...
                'K1': torch.from_numpy(K0).float().cuda(),
                'K2': torch.from_numpy(K1).float().cuda(),
                'RT': priorRT,
            ...
            kp2 = torch.FloatTensor(kpts1).cuda()
            kp1 = torch.FloatTensor(kpts0).cuda()
        ...
        for _E in np.split(E, len(E) / 3):
            n, R, t, _ = cv.recoverPose(_E.astype(np.float64), kpts0.astype(np.float32), kpts1.astype(np.float32), np.eye(3), 1e9, mask=mask)
            if n > best_num_inliers:
                best_num_inliers = n
                ret = (R, t[:, 0], n)

        return ret, inliers_best_tight, inliers_best_ultra_tight
