...
class RandomConcatSampler(Sampler):
    ...
    def __init__(self,
        ...
        print("rank", self.rank, "num replicas", self.num_replicas)

        self.data_source = data_source
        self.n_subset = len(self.data_source.datasets)
        self.n_samples_per_subset = n_samples_per_subset
        self.n_samples = self.n_subset * self.n_samples_per_subset * repeat * 1.0 // self.num_replicas
    ...
    def __iter__(self):
        ...
        # filter by rank and total size
        print("len indices before: ", len(indices), "rank / n samples / n replicas", self.rank, self.n_samples, self.num_replicas)
        indices = indices[int(self.rank*self.n_samples):int(self.n_samples*(self.rank+1))]
        print("len indices after: ", len(indices))
