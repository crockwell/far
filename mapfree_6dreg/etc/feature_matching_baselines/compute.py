...
def get_parser():
    ...
    if dataset == '7Scenes':
        ...
        if args.pred_train_set:
            print("running on train set!")
            args.scenes = [folder for folder in (args.data_root / 'train').iterdir() if folder.is_dir()]
        else:
            print("running on val and test set!")
...
if __name__ == '__main__':
    ...
    if args.dataset == '7Scenes':
        ...
        for scene_dir in args.scenes:
            ...
            for pair in tqdm(im_pairs_path):
                pts = matcher.match(pair, args.save_feats)
                pts_stack.append(pts)
            pts_stack = stack_pts(pts_stack, args.save_feats)
            results = {'correspondences': pts_stack}
            if args.save_feats:
                np.savez_compressed(scene_dir / f'correspondences_{args.matcher}_feats.npz', **results)
            else:
        ...
        results = {'correspondences': pts_stack}
        np.savez_compressed(
            f'../..//nfs/turbo/justincj-turbo/cnris/mapfree/data/baselines/correspondences_{args.matcher}_scannet_test.npz',
            **results)
